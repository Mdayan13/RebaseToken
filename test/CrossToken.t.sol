...
contract CrossChainTest is Test {
    ...
    function setUp() public {
        ...
        TokenAdminRegistry(arbitrumNetworkDetails.tokenAdminRegistryAddress).acceptAdminRole(address(sepoliaToken));
    ...
    function configureNetwork(
        ...
        TokenPool(localPool).applyChainUpdates(chainId);
    }
    ...
    function bridgeToken(
        ...
        uint256 remoteBeforeBalance = remoteToken.balanceOf(user);
        ccipLocalSimulatorFork.switchChainAndRouteMessage(remoteFork);
        uint256 remoteBalanceAfter = remoteToken.balanceOf(user);
        assertEq(remoteBalanceAfter, remoteBeforeBalance + amountTobridge);
        uint256 remoteInterestRate = localToken.getuserInterestRate(user);
